...
from aegis_cartographer.fingerprint import extract_clickable_elements, get_skeleton_hash
from aegis_cartographer.models import ActionType, Edge, MapElement, NodeStatus
from aegis_cartographer.security import PackageNameGuard, SecurityFilter
...
class TraversalEngine:
        ...
        while self.exploration_queue:
            ...
            for elem in sorted_elements:
                ...
                if new_state == current_state:
                    logger.info("      ⚠ 状态未变化，尝试 LONG_PRESS")
                    ...
                    if new_state == current_state:
                        logger.info("      ⏭ LONG_PRESS 也无变化，跳过此元素")
                        continue
                ...
                if new_state not in self.visited_states:
                    ...
                    if len(new_path) < self.max_depth:
                        ...
                        logger.info("      ⏭ 达到最大深度限制")
                else:
                    logger.info("      ⚡ 状态已存在，跳过")
                ...
                if not backtrack_success:
                    logger.info("      ⚠ 回退失败，尝试硬重置")
                    if not self._hard_reset(current_state, path):
                        logger.error("      ❌ 硬重置失败，终止探索")
    ...
    def _hard_reset(self, target_state: str, path: list[str]) -> bool:
        ...
        for edge in target_node.edges:
            ...
            self.execute_action(edge.trigger_id, edge.action_type)
            time.sleep(1.0)
